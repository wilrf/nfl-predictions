...
class OddsAPIClient:
    ...
    def _validate_request_timing(self, week_type: str):
        ...
        if week_type == "opening":
            # Tuesday morning for opening lines
            if day != 1 or hour < 5 or hour > 8:
                raise OddsAPIError(f"Opening lines should be fetched Tuesday 6-8am (currently {now.strftime('%A %I%p')})")

        elif week_type == "midweek":
            # Thursday evening
            if day != 3 or hour < 17 or hour > 20:
                raise OddsAPIError(f"Midweek update should be Thursday 5-8pm (currently {now.strftime('%A %I%p')})")

        elif week_type == "closing":
            # Sunday morning before games
            if day != 6 or hour < 8 or hour > 11:
                raise OddsAPIError(f"Closing lines should be Sunday 8-11am (currently {now.strftime('%A %I%p')})")

        elif week_type == "current":
            # Saturday night check
            if day != 5 or hour < 22:
                raise OddsAPIError(f"Current update should be Saturday after 10pm (currently {now.strftime('%A %I%p')})")
