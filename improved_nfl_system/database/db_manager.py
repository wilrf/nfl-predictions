...
class NFLDatabaseManager:
    ...
    def insert_suggestion(self, suggestion: Dict) -> int:
        """Insert betting suggestion - FAIL if invalid"""
        # Validate confidence range
        if not (50 <= suggestion['confidence'] <= 90):
            raise DatabaseError(f"Confidence {suggestion['confidence']} not in 50-90 range")

        # Validate margin range
        if not (0 <= suggestion['margin'] <= 30):
            raise DatabaseError(f"Margin {suggestion['margin']} not in 0-30 range")

        # Validate minimum edge
        if suggestion['edge'] < 0.02:
            raise DatabaseError(f"Edge {suggestion['edge']} below 2% minimum")

        # Validate kelly fraction
        if not (0 < suggestion['kelly_fraction'] <= 0.25):
            raise DatabaseError(f"Invalid Kelly fraction: {suggestion['kelly_fraction']}")
